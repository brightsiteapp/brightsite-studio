...
assert.equal(new Set(layouts.map(item=>item.name)).size,13);
assert.equal(layouts.map(item=>item.id).join(','),'editorial,bold,kinetic,serene,index,studio,flux,bloom,torque,minimal,luxe,soft,organic');
